...
## Usage
    java -jar log-combiner-1.0.5.jar <folderAbsolutePath> [configAbsolutePath]

## Examples
Combine all files in the `/var/log/temp` folder recursively.

    java -jar log-combiner-1.0.5.jar /var/log/temp
...
will be used and files will be joined by last modification time of files.
    ...
    java -jar log-combiner-1.0.5.jar /var/log/temp /var/log/combiner/two-apps.properties
...
* `log-combiner-1.0.5.jar` (mandatory) is an executable jar file. Latest release is located on the Maven Central Repository.
